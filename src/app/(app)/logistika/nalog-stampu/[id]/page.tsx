...
export default function NalogStampuDetail() {
  ...
  const { data, isLoading, error, refetch } = trpc.nalogStampu.byId.useQuery({ id });
  ...
  const vratiNaKorekciju = trpc.nalogStampu.vratiNaKorekciju.useMutation({ onSuccess: () => refetch() });
  const odobriKorekciju = trpc.nalogStampu.odobriKorekciju.useMutation({ onSuccess: () => refetch() });
  ...
  if (error) return (
    <div className="rounded-md border border-destructive/40 bg-destructive/5 p-4">
      ...
      <p className="mt-1 text-xs text-muted-foreground">Ako je &quot;Tenant mismatch&quot;, nalog je u drugom pravnom licu nego što je tvoja sesija.</p>
    </div>
  );
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {data.status !== "PROBLEM" && (
            <Button size="sm" variant="outline" onClick={() => {
              const razlog = prompt("Razlog popravke (biće poslat štampariji):");
