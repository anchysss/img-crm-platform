...
export const nalogMontazuRouter = router({
  ...
  create: withPermission("campaigns", "CREATE").input(
    ...
    // Generiši broj naloga: <RN.broj>-MONTAZA-<TIP>-<n>
    ...
    const created = await prisma.nalogMontazu.create({
      data: {
        ...
        grad: input.grad,
        ekipa: input.ekipa,
        datumMontaze: input.datumMontaze,
        napomena: input.napomena,
        stavke: { create: input.stavke.map((s) => ({ ...s, brojPanoa: s.brojPanoa ?? null })) },
      },
      include: { stavke: true },
    });
  ...
  addStavka: withPermission("campaigns", "UPDATE").input(
    ...
    return prisma.nalogMontazuStavka.create({
      data: { nalogId, ...rest, brojPanoa: rest.brojPanoa ?? null },
    });
  }),
